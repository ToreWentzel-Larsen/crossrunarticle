...
Together with the constancy condition $pd=(1-p)u$ this gives $d=1-p, u=p$. The autocorrelation k between two subsequent observations may be computed as $k=1-\frac{d}{1-p}=1-\frac{u}{p}$. Thus this is a case in which independence is equivalent with $k=0$.

The main interest in the joint distribution of C and L may be in the symmetric case, but the procedure outlined here is quite general, only assuming the constancy condition. We therefore investigate the 
