...
% Jeg er usikker på dette afsnit. Særligt anden sætning. Hvordan kan første værdi være f=2?
...
Case 1 is the simplest one. Then no runs within the last $n - g$ is longer than the initial run and the total number of crossings is one more than the number of crossings within these last part of the sequence. Therefore non-zero numbers are confined to $1 \leq c \leq n-g, l=g$ and 

$$\text{NFI}_{n,m} (C=c, L=l, G=g) = \text{NFN}_{n-g,m-g} (C=c-1)$$
