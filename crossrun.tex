...
% ToRe: Må skrive om, dette er forkortet og ligger i Limitations and planned extensions nå
Finally, the case where the median is determined from the data itself is considered and a similar procedure in that case is outlined. This case is not yet included in the \code{crossrun} package.
...
% ToRe: figuren kan ikke legges "here" [h...] på grunn av plassmangel, så da legges den "top", 
% øverst på neste side. Det betyr at "aqbove" må tas vekk i et avsnitt nedenfor.
...
%ToRe: E>rstattet kolon med below, det er ikke plass til figuren rett under så den kommer øverst på neste side
...
Another important limitation is that the iterative procedure does not apply for autocorrelated time series. Here also, work is in progress in a simple autocorrelation model in which the probabilities of "success" and "failure" in each observation may depend on the previous observation only, and a function \code{crossrunauto} is planned for inclusion in a future update. The practical value of this procedure is likely to be limited  since decision rules based on the number of crossings and the longest run are probably not particularly useful for autocorrelated series, but at leasty the procedure may be used to investigate the extent of the problem.
